...
plot(knn_cv)
```
We did several confusion matrix to measure the performance. The first one was with knn method. We found an accuracy of 0.615 which corresponds to well predicted clients (good and bad). The best accuracy value is 1 so 0.615 is considered as a not bad value. 
...
```
The cinfusion matrix presents an accuracy of 0.745.
...
By looking at this summary table of the models, we can see that Support Vector Machines, Tree and Logistic Regression are our top models. However, in terms of sensitivity and specificity, two models outperform the other. As a reminder, in our case, we want to be sure to have an balanced sensitivity and specificity. Therefore, we take only the two models for the following part: 
